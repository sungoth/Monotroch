...
__weak unsigned char UartPutc(unsigned char my_ch)
{
	if(HAL_UART_Transmit(&huart6,&my_ch,1,5000) == HAL_OK)
		return my_ch;
	else
		return 0;
}
__weak unsigned char UartGetc(void)
{
	uint8_t ch;
	if(HAL_UART_Receive(&huart6,&ch,1,5000) == HAL_OK)
		return ch;
	else
		return 0;
}
